...
export default class RazorGenerator{

    static readonly instance=new RazorGenerator();
     ...
     * @param template 
     */
    converToCode(template:string):string{
        //将模板转换成代码
        //遍历文本，进行语法块分析

        let putResult:string="";
        let put:string="";//存放常规文本内容
        let blockList:string[]=[];//处理嵌套的逻辑组
     ...
     */
    runTemp(funcStr:string,data:{[key:string]:any}):string|null{
        ...
        try{
            ...
            console.log(chalk.blue(funcStr));
            return null
        }
    }
     ...
     * ```
     */
    protected mathBracket(str:string,start:number):number{
     ...
     * {为被匹配的起点
     */
    protected mathBigBracket(str:string,start:number):number{
        //循环，忽略引号内的括号，找到结束的括号点
        let count=0;//当前匹配到{的数量
        let inQuotationMark=false;//当前是否在引号内
        let quotationMark=null;//当前所在的引号符号
     ...
     * 查找变量结构 可以有函数调用
     */
    protected mathVar(str:string,start:number):number{
        for(let i=start;i<str.length;i++){
            let char=str[i];
