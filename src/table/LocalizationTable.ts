...
export default class LocalizationTable{
     ...
     * @param value 值
     */
    setValue(key:string,lang:string,value:string,style?:{color?:string,background?:string,describe?:string}){
        let keys=this.sheet.getColumValue(0);
        let langs=this.sheet.getRowValue(0);
        if(!keys||!langs){
    ...
    setKeyDescribe(key:string,describe:string){
        let keys=this.sheet.getColumValue(0);
        let langs=this.sheet.getRowValue(0);
        if(!keys||!langs){
            this.sheet.setValue(0,0,"key");
     ...
     * @param color 颜色值
     */
    setRowBackground(key:string,color:string){
        let keys=this.sheet.getColumValue(0);
        if(!keys)
            return;
        let keyIndex=keys.indexOf(key);
     ...
     * @param lang 语言
     */
    extractLangeData(lang:string):{[key:string]:string|null}{
     ...
     * @param localTable 
     */
    putNewTable(localTable:LocalizationTable,defaultLang:string){
        ...
        for(let oldk of oldKeys){
            if(newKeys.indexOf(oldk)==-1){
                ...
                console.log(chalk.bgRed("lose key",oldk))
            }
        }
        ...
        for(let newk of newKeys){
            if(oldKeys.indexOf(newk)==-1){
                ...
                console.log(chalk.bgYellow("add key",newk))
            }else{
                //判断值是否有变化 有变化的文字标蓝色
                ...
                if(oldValue!=newValue){
                    ...
                    console.log(chalk.blue(oldValue!,">",newValue!));
                }
            }
        }
